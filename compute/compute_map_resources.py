...
Validation pipeline:
  parser/parse_replay_aggregates.py → docs/derived/replay_ground_truth.json
...
Trees per cluster come from `docs/derived/pattern_type_stats.json`
...
def write_report(r: dict, settings: dict) -> str:
    ...
    L.append(f"**Настройки:** `mapsize = {settings['mapsize']}` ({r['map_name']}, {r['dim']} × {r['dim']} = {r['area']} тайлов), "
             ...
             f"Поведение движка по каждой опции — [`recon/game_settings.md`](../../recon/game_settings.md).")
    L.append("")
