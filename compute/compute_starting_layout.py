...
def render_legacy(circles: dict, grid: dict, presets: list[dict]) -> str:
    ...
    A("")
    A("На карте «Суша» генератор принудительно задаёт `foreststype = 0` и "
      "использует pinefir/spruce/pine (хвойные наборы). Для других типов "
      "местности параметр не следует считать принудительно нулевым. "
      "В пустынном сезоне вместо forests используются паттерны `desert_forests_*`.")
    A("")
    ...
    A("Игрок выбирает один из этих режимов в лобби. **default** (id=0) — это то, "
      ...
      "юнитов). Остальные режимы заменяют эту группу собственными ресурсами, "
      "юнитами и зданиями из cfg-файла.")
    A("")
    ...
    A("")
    A("**Замечания:**")
    A("- Ресурсы — это **прибавка** к стандартным стартовым 0/0/0/0/0/0. "
