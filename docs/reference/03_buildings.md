...
Прежде чем переходить к таблицам, зафиксируем четыре механики, общие для **всех** зданий — стройка, ремонт, отмена, разрушение. Источник: [`recon/world/building_mechanics.md`](../recon/world/building_mechanics.md). Все ссылки на код собраны в разделе [Источники](#источники) в конце.
...
- **Отмена недостроенного здания (Foundation).** GUI-обработчик `_misc_GUICancelBuilding` [^1] вызывает `GameObjectDestroyByHandle`, после чего здание идёт по обычной death-цепочке. Возвращаются 100% потраченных ресурсов (поведение в игре). Зеркального `_res_AddResToPlayerByIndex` к `_unit_ApplyCostByID` [^2] в скриптах не найдено — refund foundation cost, видимо, обрабатывается на стороне C++ (см. [`recon/world/building_mechanics.md §8`](../recon/world/building_mechanics.md#8-резюме-механик-быстрый-ответ-на-частые-вопросы)).
- **Отмена заказа юнита в очереди.** `_unit_CancelUnitProduction` [^3] возвращает `price[k] × costmodifier`, где `costmodifier = pow(costpercent/100, restype)` и `restype` — счётчик built-копий, сохранённый в `OrderInfo` в момент заказа [^4]. На практике это 100% от того, что было реально списано в момент заказа, независимо от того, какая цена у этого юнита будет на текущий момент.
- **Отмена апгрейда.** `_unit_CancelUpgradePerform` [^5] возвращает базовую стоимость апгрейда из `_country_GetUpgradeCostBySID`. Costpercent-масштабирования у апгрейдов в этой ветке нет, поэтому возврат равен списанному.
...
- При `hp ≤ 0` или `bDie := True` здание получает `essential_death`. State-машина [^6] ставит первый таймер `DelayExecuteState`:
  - если здание было `essential_birth` (отмена недостроенного) — сразу `DeathStage2` через `gc_building_deathtime_1 = 30` g-сек;
  - иначе — `DeathStage1` через `gc_building_deathtime_0 = 30` g-сек, далее `DeathStage1` [^7] меняет mesh на `<sid>_death1` и ставит `DeathStage2` ещё через 30 g-сек. Для `usage = mine` обе паузы удваиваются (60 g-сек каждая).
- **Корпус** в этом промежутке остаётся на карте: визуально — mesh `<sid>_death1.mesh`, в состоянии `essential_death`, материал `'debris'` [^8], коллизия — прежняя. Постройка нового здания на этих клетках невозможна, пока корпус не исчезнет.
- **Сброс клетки.** В `DeathStage2` [^9] для не-стен с `gc_collisiontag_terrain` вызывается `_unit_SetTerrainCollision(myHnd, gc_collisiontag_none)` + `_misc_UnitTopologyUpdate`, затем `GameObjectRequestToDestroyByHandle`. После этого клетка освобождается, и на ней можно ставить новое здание.
- **Гарнизон внутри** (если у здания `peasantabsorber > 0` или `transport > 0`). `_unit_DestroyObj` [^10] собирает `gc_argunit_inside` и вызывает `_unit_DoUnitsGoOutside(list, bDead=True, ...)`. Эта процедура [^11] ставит каждому юниту в списке `essential_death`, то есть содержимое умирает вместе со зданием.
...
- **OnDeath-хук** [^12] до удаления прокручивает очередь заказов здания: `produce`-заказы прогоняются через `_unit_ProduceUnit(... bState=False, ...)` [^13], что внутри ведёт к `_unit_CancelUnitProduction` и возврату ресурсов; `performupgrade`-заказы — через `_unit_CancelUpgradePerform`. То есть при сносе работающей казармы или академии ресурсы за уже оплаченные юниты и апгрейды возвращаются игроку, а не сгорают.

## Источники

Все ссылки относительно `data/scripts/` в установке Cossacks 3.

[^1]: `_misc_GUICancelBuilding` — `miscext2.script:3898-3953`.

[^2]: `_unit_ApplyCostByID` — `unit.script:5728-5762`.

[^3]: `_unit_CancelUnitProduction` — `unit.script:5891-5977`.

[^4]: Комментарий автора про `restype` — `unit.script:6017`: *restype stores amount of units, that we had at the moment of ordering produce*.

[^5]: `_unit_CancelUpgradePerform` — `unit.script:5837-5889`.

[^6]: State-машина смерти здания — `units/building.inc/settagstates.inc:32-53`.

[^7]: `DeathStage1` — `units/building.inc/deathstage1.inc:5-11`. `DeathStage2` определён в `deathstage2.inc`.

[^8]: Установка материала `'debris'` для трупа здания — `units/building.inc/ontagstates.inc:286`.

[^9]: `DeathStage2` — освобождение клетки и финальное удаление — `units/building.inc/deathstage2.inc:8-15`.

[^10]: `_unit_DestroyObj` — `miscext2.script:4232-4242`.

[^11]: `_unit_DoUnitsGoOutside` — `unit.script:4559-4564`.

[^12]: OnDeath-хук здания — `units/building.inc/ondeath.inc:11-25`.

[^13]: `_unit_ProduceUnit` — `unit.script:10351`.
