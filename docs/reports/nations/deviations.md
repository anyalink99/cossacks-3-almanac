...
Этот отчёт **не дублирует**, а дополняет [`reports/nations/overview.md`](overview.md). overview даёт обзор «у кого что есть» (roster size, building coverage, рынок-кластеры) и top-10 stat-anomalies по HP-разбросу. Здесь же перечисляются полные стат-отпечатки.
...
Берётся каждый sid юнита, у которого есть запись хотя бы у двух наций (если у одной — это уникальный юнит, описывается в [`reports/nations/overview.md`](overview.md) §3). Записи группируются по стат-отпечатку (HP / buildtime / цена / щит / скорость / защиты / consume / weapon-набор). Юниты с одинаковым отпечатком сливаются в одну группу.
