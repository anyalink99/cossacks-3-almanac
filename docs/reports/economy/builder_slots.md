...
⚠️ **Ворота** (`*sga`, `*wga`, `*sga_*`, `*wga_*`) не строятся крестьянами. Игрок выделяет существующий участок стены и кликает «превратить в ворота» — слоты в таблицах ниже информативны, но в обычной игре не используются.
...
## Ворота (sga/wga) — не строятся крестьянами
