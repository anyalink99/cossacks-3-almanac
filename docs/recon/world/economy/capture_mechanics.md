...
- Через заданное число игровых тактов движок измеряет евклидово расстояние от центра
...
Метрика — квадрат **евклидова расстояния** [^3]: `(px, py)` — позиция объекта-жертвы,
`(tx, ty)` — позиция кандидата-захватчика. Это **центр-к-центру**, ни
манхэттенское расстояние, ни расстояние Чебышёва. Форма здания
не учитывается: используется одна опорная точка.
...
[Настройки матча](../../../reports/map/lobby_settings.md#capture--правила-захвата).
Взаимодействие с мирным временем (`peacetime`) и владением территорией
описано в [Как настройки матча влияют на игру](../map/game_settings.md) §3.4.
...
- `SetObjBuildingBaseSettings(objprop, True, …)` — здание ловится:
  - Склад (`commonsid+'sto'`);
  - Шахты (`commonsid+'gol'/'iro'/'coa'`);
  - Городской центр (`csid+'cen'`, `gc_obj_usage_center`);
  - Казармы 17 и 18 века (`csid+'bar'`, `csid+'ba2'`);
  - Мельница (`csid+'mil'`): блок не переопределяет `bcapture`, а
    получает значение от вызывающего кода;
  - Кузница (`csid+'bla'`);
  - и др. через `SetObjBuildingExtProperties(... True, ...)`
- `False` (= не захватываются, только разрушаются):
  - Башня (`commonsid+'tow'`): **башни нельзя захватить**, только разрушить;
  - Порт (`commonsid+'por'`);
  - стены и ворота (`commonsid+'swa/sga'`, `ukrwwa/wga`);
  - Конюшня (`csid+'sta'`);
  - Академия (`csid+'aca'`);
  - Дипломатический центр (`csid+'dip'`);
  - национальные здания 17 и 18 века (`csid+'17'`, `csid+'18'`);
  - объекты миссий (`misblg/misblg2`, `misyurt`, `miscommandcenter`).
...
`SetupStartingResources` (см. [разбор генерации карты](../map/map_generation_pipeline.md))
...
- У Башни нет мест для гарнизона (`peasantabsorber=0`, `transport=0`,
  см. [^21] и [раздел о Башне](building_mechanics.md#53-башня-как-самостоятельное-орудие)
  в статье о [строительстве и ремонте](building_mechanics.md)), поэтому вопрос
...
⇒ Чтобы захватить здание, пехотинцу нужно подойти **к опорной точке
здания** на расстояние менее 4,013 клетки по евклидовой метрике. Если
здание занимает большую площадь, например Городской центр 5×4, захватчик может
стоять дальше «края» — функция использует **точку центра**
(`game-object position`), а не границы модели (`bbox`). Эта точка обычно близка
...
1. **Точная позиция (`px`, `py`) у здания:** центр модели, центр
   ...
   построить Казарму, подвести Крестьянина к краю и измерить минимальное
   расстояние до события захвата.
2. **`bAutoKill`** на третьем шаге — переменная объявлена, но никогда не
   присваивается в `_misc_CheckCapture`. Возможно, это унаследованный
   код первой игры; сейчас значение всегда остаётся `False`.
3. Для стены (`wall`) `_unit_SearchCapturersForWall` не требует
...
5. Как проверка взаимодействует с туманом войны? Предположительно,
...
[^6]: Здания с `bcapture` в `_unit_InitBase` — `lib/unit.script`.
      При `bcapture=True`: 2205 (Склад), 2312 (Шахты), 2371 (Городской
      центр), 2421 (Казарма), 2514 (Кузница). При `bcapture=False`: 2153
      (Порт), 2224 (Башня), 2259/2286 (стены и ворота), 2452
      (Дипломатический центр), 2462/2472 (национальные здания XVII и
      XVIII веков), 2493 (Академия), 2503 (Конюшня), 2540 (объекты миссий).
