...
почему любой вражеский объект, не являющийся зданием, может не захватить
сегмент, а разрушить его. Внутренние структуры и последовательности вызовов вынесены в
...
- Любой вражеский объект, не являющийся зданием, на расстоянии четырёх
  клеток — в том числе Крестьянин или артиллерия — **не получает стену во
  владение, а уничтожает сегмент** [^11]. Дружественный защитник примерно
  в восьми клетках предотвращает снос. Если здоровье сегмента
...
В игре есть Частокол и Каменная стена [^1]. Частокол доступен всем
21 нациям. Каменная стена есть у всех, кроме Украины; её внешний вид и
параметры различаются у европейских наций, России, Турции и Алжира [^6].

| Вариант | Доступность | Здоровье стены / ворот | Цена сегмента | Время постройки | Содержание сегмента |
|---|---|---:|---:|---:|---:|
| **Частокол** | все нации, кроме особого украинского варианта | 1500 / 1000 | 10 дерева | 18 кадров | нет |
| **Украинский Частокол** | Украина | 2500 / 1500 | 12 дерева | 26 кадров | нет |
| **Европейская Каменная стена** | все, кроме Украины, России, Турции и Алжира | 50 000 / 32 000 | 50 камня | 288 кадров | 0,4 камня/с |
| **Русская Каменная стена** | Россия | 50 000 / 32 000 | 60 камня | 640 кадров | 0,32 камня/с |
| **Турецкая Каменная стена** | Турция и Алжир | 50 000 / 32 000 | 60 камня | 384 кадра | 0,24 камня/с |

Содержание Каменной стены списывается постоянно, пока сегмент стоит [^1].
За игровую минуту один европейский сегмент расходует 24 камня, русский —
19,2, турецкий — 14,4. Украина имеет только Частокол; Каменной стены у
этой нации нет.
...
```
[стена][стена][стена]   ← 3 клетки линии
```
...
Обычное захватываемое здание меняет владельца, если вражеский юнит,
способный захватывать здания, подходит на четыре клетки и рядом нет
защитников. Для стен и ворот действует особое правило [^11]:
...
1. Игра ищет поблизости любой подходящий вражеский объект, который не
   ...
   условию соответствуют в том числе Крестьяне и артиллерия.
2. Если здоровье сегмента меньше трети от максимума, дальнейшая логика
   пропускается: сегмент уже добивают оружием, без эффекта захвата
   [^12].
3. Дружественный юнит-защитник примерно в восьми клетках отменяет снос.
4. Иначе при найденном противнике сегмент мгновенно уничтожается.

Правило действует и для готового, и для недостроенного сегмента, **пока у
него не меньше трети максимальной прочности**. Ниже этого порога проверка
сноса через захват пропускается, и сегмент нужно добить оружием. Владелец
никогда не меняется.
...
| Игровой вариант | Стена / ворота | Класс и материал |
|---|---|---|
| Частокол | `ukrwwa` / `ukrwga` | `gc_obj_usage_weakwall`, `gc_obj_material_woodwall` |
| Европейская Каменная стена | `eurswa` / `eursga` | `gc_obj_usage_hardwall`, `gc_obj_material_building` |
| Русская Каменная стена | `russwa` / `russga` | `gc_obj_usage_hardwall`, `gc_obj_material_building` |
| Турецкая Каменная стена | `turswa` / `tursga` | `gc_obj_usage_hardwall`, `gc_obj_material_building` |

У объектов стены поля `buildtime` и `consume.stone` задают время
постройки и постоянное содержание. Значения `consume.stone` равны
250 у европейского, 200 у русского и 150 у турецкого варианта;
в секунду списывается `consume.stone × 32 / 20000`. Ворота отличаются
полем `bgate = True`; готовность обычной стены хранится в `bbuilt`.
...
`_unit_SearchCapturersForWall` требует только вражеского владельца и
`not bbuilding`, но не `bcancapture`. Поэтому кандидатами становятся в том
числе Крестьяне и артиллерия. Затем `_unit_SearchProtectors` ищет
дружественный объект с `bprotector` в радиусе
`gc_gameplay_protectionradius` (примерно восемь клеток); его присутствие
сбрасывает `bcapture`. Если защитника нет, ветка `bwall` устанавливает
`bDie := True` и `gc_statetag_essential_death`; при `hp < maxhp / 3` эта
проверка пропускается [^11][^12].
...
[^8]: `data/scripts/lib/misc.script:3998-4058` —
      ...
      выполнены; иначе `-1` (улучшение недоступно).
...
[^11]: `data/scripts/lib/miscext.script:961-1185` —
       ...
       (без требования `bcancapture` у захватчика).
       `data/scripts/lib/unit.script:4666-4691` показывает точное условие:
       кандидат должен быть вражеским объектом с `not bbuilding`; поэтому
       подходят и Крестьяне, и артиллерия. Линии
       `miscext.script:1041-1068` ищут дружественный `bprotector` в пределах
       `gc_gameplay_protectionradius` и при его наличии сбрасывают
       `bcapture`. Линия 1106 —
