...
- Сегмент стены — 1 × 1 клетка. Длинная стена — последовательность
...
- Захват сегмента стены (или ворот) пехотой противника на
  `gc_gameplay_captureradius = 4` клетки **не передаёт владельца**, а
...
клетки от объекта `bcapture = True` без своих защитников, объект
