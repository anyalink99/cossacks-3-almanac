...
- Поиск идёт через **сетку поиска** (`scan-grid`) [^1]: карта
  разбита на ячейки фиксированного
...
- **Атака с движением** для пехоты и кавалерии — это приказ
  `gc_obj_order_type_move`
...
Эту функцию автомат состояний юнита вызывает примерно каждые 100 мс
...
### 4.2 Выбор режима поиска (`scanmode`)
...
- **Стрелок в движении эффективно «ниже» на 3 тайла** по радиусу
  ...
  `≈ 0.1` — 0.3 тайла. То есть в одной шеренге часть Мушкетеров
...
- **Диверсант под управлением ИИ целит в максимальный урон.** В
  ...
  не «обычный» ИИ; см. [`ai_behavior.md`](../../systems/ai_behavior.md), раздел
  «Открытые вопросы».

---

## 7. Лечение духовными лицами (`bpriest`)

Капеллан (`priest`), Поп (`pope`), Мулла (`mullah`) и Падре
(`padre`) — особый класс
...
«Атака» лекаря обрабатывается отдельной веткой
...
| Юнит | Лечение за удар | Дальность (пиксели / тайлы) | Где доступен |
|---|---:|---|---|
...
### 9.1. Базовые значения разброса (`dispertion`)

| Оружие | Разброс (пиксели / тайлы) | Отклонение на 15 тайлах |
|---|---:|---:|
| Стрелец, мушкетный выстрел (`SHOTMUSKET`) | 200 / 3,75 | ±1,50 тайла |
| Лучник, стрела (`STRELA`) | 175 / 3,28 | ±1,31 тайла |
| Лучник, огненная стрела (`OSTRELA`) | 200 / 3,75 | ±1,50 тайла |
| Мушкетер, базовое значение | 250 / 4,69 | ±1,88 тайла |
| Пушка, ядро (`PPOINTT`) | ~250 / 4,69 | ±1,88 тайла |
| Башня, ядро (`PPOINTTTOW`) | ~100 / 1,88 | ±0,75 тайла |
| Яхта / Галера, ядро (`PPOINTTKOR`) | 25 / 0,47 | ±0,19 тайла |
...
Пример: Мушкетер (`disp = 3.75`) на 15 тайлах →
...
### 9.3. Улучшения разброса (`dispertion`)
