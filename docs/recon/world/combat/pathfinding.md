...
поиска пути живёт в нативном движке (C++); скрипты только ставят юнитов
...
- [Игровые такты](../../../../internals/engine/ticks_and_subticks.md) —
  главный цикл расчёта (`gc_progress_Interval = 0.02 s`), шаг расчёта
  юнитов — 100 мс.
- [Синхронизация сервера](../../../../internals/engine/server_sync_architecture.md) —
  `WriteMove` / `ReadMove`, модель с решающим сервером и сохранение
  очереди в файл игры.
- [Механика зданий](../economy/building_mechanics.md) — занимаемая
  площадь и масса (`CIMass`) зданий при столкновениях.

## Коротко

- Движение юнита — это **две независимые подсистемы**: глобальный поиск
  пути из точки A в точку B по карте препятствий (`QuadTree`) и локальное
  предотвращение столкновений (`CollisionInertia`). Обе живут в
  нативном движке; в скриптах — только запросы к ним.
- Поиск пути выполняется **пакетом** раз в 20 мс (один такт `progress`):
  движок берёт всю очередь юнитов и рассчитывает маршруты одним вызовом.
...
- **Построение** задаёт каждому юниту собственную точку с небольшим
...
## 2. Где и как рассчитывается путь

### 2.1 Очередь и пакетная обработка
...
### 2.2 Главный пакет запросов (раз в 20 мс)
...
базовой ячейки столкновений **в тайлах** (то есть одна ячейка =
...
## 4. Предотвращение столкновений (`CollisionInertia`, `CI`)
...
### 4.1 Параметры отдельного юнита
...
**Вывод:** радиус столкновения пешего юнита — около 0,16 тайла, то есть
маленький, юниты могут плотно стоять. Лошади получают `unitradius=15`
визуальный радиус.

### 4.2 Корабли получают большую инерцию столкновений
...
### 4.3 Здания — тяжёлые неподвижные препятствия
...
[строительстве и ремонте](../economy/building_mechanics.md)). Глобальный путь вокруг
здания строит глобальный поиск пути; локальная система столкновений лишь
страхует, чтобы
юнит не упёрся в стену вплотную.

### 4.4 Расталкивание юнитов и передний сектор в 90°
...
## 5. Обработка застревания

### 5.1 Проверка каждый кадр: «нет пути — остановиться»
...
### 5.4 Аварийное исправление зависания
...
### 5.5 Проверка при загрузке: «два юнита в одной точке»
...
### 6.4 Номер отряда как подсказка движку
...
## 7. Когда путь рассчитывается заново
...
### 8.3 Глобальное ограничение цикла расчёта
...
2. **Влияние числовой метки отряда:** действительно ли движок использует
   ...
   параллельно через узкий проход шириной в один тайл и посмотреть,
   конфликтуют ли пути.
