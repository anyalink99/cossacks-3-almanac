...
- Генератор использует смешанные хвойные леса; пустынный сезон заменяет их
  отдельными пустынными наборами окружения [^2].
- В финале всегда появляются **18 крестьян** в сетке 6 × 3, независимо от
  варианта стартовой армии.
...
Если `gMap.settings.additional.startingunits > 0` (не «По умолчанию») → вместо 18 крестьян вызывается `CreateUniqueStartingUnits` (нация-специфичный отряд: офицер + барабанщик + несколько пехотинцев). Все 14 пресетов с каноническими русскими названиями — [Настройки матча](../../../reports/map/lobby_settings.md#startingunits--стартовая-армия); поведение — [Как настройки матча влияют на игру](game_settings.md) §3.1.
...
| [`compute/validate_map_predictions.py`](../../../../compute/validate_map_predictions.py) | Запускает для каждого реплея `compute_counts(...)`, сравнивает прогноз с фактом и строит [таблицу калибровки](../../../../internals/data/map_predictions_validation.md) |
...
`foreststype` сначала получает `floor(RandomExt*3)`, затем сразу
перезаписывается значением `0` [^2]. Ветка 0 использует наборы
