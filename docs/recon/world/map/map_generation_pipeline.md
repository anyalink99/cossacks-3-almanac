...
остальные ресурсы и завершает карту. Технически эту цепочку выполняет
`DoGenerate`, запущенная через `ExecuteState('DoGenerate')` [^1]. Ссылки на
код и блоки Pascal собраны в разделе [Источники](#источники) в конце
документа.
...
- `cBorderObjDist = 1` — шаг пограничных объектов времени мира в клетках.
