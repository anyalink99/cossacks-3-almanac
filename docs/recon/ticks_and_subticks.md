...
В мультиплеере: server рассылает изменение через `LanSendParser(gc_LAN_GAME_SYNC_GAMETIME, ...)` — все клиенты подстраиваются.
...
- **Текущая фаза анимации** — `OnBeforeSave` зовёт `SwitchTo('Nothing')`, что рвёт текущий анимационный цикл. После Load крестьянин стартует анимацию с нулевой фазы (или с подобранной enginом).
