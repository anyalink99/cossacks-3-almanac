...
- **Первая наступательная волна** состоит из пяти отрядов. Когда они
  собраны, компьютер отправляет их к ближайшей цели.
...
На обычном уровне диверсий нет, на сложном одновременно готовятся две
группы, на очень сложном и невозможном — четыре [^13]. На лёгком уровне
диверсий также нет. На очень
...
9. **Дальности `gc_ai_dist_mines_lvl1 = 30` / `lvl2 = 60` /
   `expansion = 85`.** Это в клетках. Стоит верифицировать, какой
...
[^17]. Размер диверсионного набора хранится в `numdiver`, а предел связан
с `gc_ai_MaxDiverArmies` [^13].
