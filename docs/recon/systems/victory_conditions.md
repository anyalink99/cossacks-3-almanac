# Recon: условия победы и конец партии

Все пути к скриптам ниже — относительно `data/scripts/` в установке Cossacks 3.

## TL;DR

- Главная функция, разрешающая исход партии — `_misc_CheckEndGame` в
  `lib/miscext2.script:3770`. Она вызывается, когда меняется состояние
  «жив ли игрок» (§4) или когда игрок капитулирует (`_misc_Surrender`,
  тот же файл, строка 3849).
