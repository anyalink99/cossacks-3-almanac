...
- Альтернативных условий — победы Чудом света, по очкам, по времени или
  захватом флага — в скриптах **нет**. В обычной партии побеждает
  последняя оставшаяся команда, а миссии используют сценарные правила (§3).
- `farmused = 0` означает поражение, но `farmused` не падает до нуля, пока есть хоть
  один крестьянин **или** Городской центр.
- Очки копятся только для статистики и рейтинга, на исход партии не влияют
  (§5.4).
...
- **Кампания** — построена поверх сценарной системы; константы
  `gc_ach_campaign_finalaus..finalrus` и каталог
  `data/game/var/campaigns.cfg` [^8]. Победа и поражение записываются в
  `TCampaignProgress.finished`, `lose`, `maxfinishdifficulty` [^9].
- **Редактор** (`gc_gamemode_editor`) и **редактор сценариев**
  (`gScenario.bactive`).
- **Сетевая рейтинговая битва** — флаги `gMap.brating`,
  `gInternetShell.bratingroom`, `bautosearch` (см. §6).
- **Реплей / наблюдение** — режимы без управления армией.

## 3. Условия победы

### 3.1 Обычная случайная карта
...
То есть действует правило **последней оставшейся команды**: игроки одной команды (по полю
...
присваивает — никто не побеждает; это позволяет свободно строить без противника.

Все игроки победившей команды получают состояние `win`, даже если ранее
кто-то из них получил `lose` из-за капитуляции [^11].

### 3.2 Сценарий и кампания

Пользовательские сценарии могут выдавать победу (`win`) или поражение
(`lose`) действиями
...
игроку), **поэтому в пользовательских миссиях нет единого правила победы
для нескольких игроков**: каждый клиент выставляет результат отдельно.

Доступные сценарные условия [^14] включают `unitsCountInZone`,
`playerResources`, `counterValue`, `timerFinished`, `flagActive` и так
далее. Поэтому захват флага, накопление 50 000 золота, удержание зоны
или убийство командира реализуются комбинацией сценарных правил. В движке
предусмотрены только два завершающих действия: `endgame_win` и
`endgame_lose`.

### 3.3 Режимы победы из других стратегий, которых в C3 нет
...
| Тема | Значение |
|---|---|
| **Обычная победа** | Остаётся одна команда (`_misc_CheckEndGame`). |
| **Обычное поражение** | `farmused = 0` и нет принадлежащего игроку крестьянина или Городского центра. В Исторической битве отключено. |
...
| **Капитуляция** | `_misc_Surrender` ставит `bleave = True`, `victorystate = lose` и вызывает `_misc_CheckEndGame`. Первый вышедший в первые 15 минут теряет рейтинг. |
| **Сценарные правила** | `endgame_win = 47` и `endgame_lose = 48` применяются только к текущему игроку (`myplind`). |
| **Режимы** | Внутренние состояния: `mainmenu`, `game`, `editor`, `spectator`, `replay`, `endgamestatistics`. Варианты партии: случайная карта, Историческая битва (`gMap.bbattle`), кампания, пользовательский сценарий и рейтинговая игра (`gMap.brating`). |
