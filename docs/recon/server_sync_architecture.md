# Recon: Cossacks 3 — серверная архитектура и сетевая синхронизация

Цель: задокументировать модель сетевой синхронизации в C3 — кто симулирует, что и когда передаётся, как это связано с тиками и random'ом. Третий документ в связке с [determinism_audit.md](determinism_audit.md) (RNG-сайты) и [ticks_and_subticks.md](ticks_and_subticks.md) (модель времени). Без понимания этого слоя нельзя объяснить почему даже синхронизация adaptive speed между хостами не делает поведение воспроизводимым.
