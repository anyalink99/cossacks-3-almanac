...
Это **QuadTree-based collision world** с двумя «слоями приоритета»: один включает все статические препятствия, другой — только terrain/walls (юниты-через-здания тащат разные приоритеты, чтобы можно было найти путь «как будто здания нет»).
...
Здесь — **главный отвечающий за «обход друг друга»** механизм. Это локальный физический slip между юнитами (push с массами и инерцией), накладываемый поверх готового пути.
...
Поведенческие радиусы (используются `WriteMove` для дисперсии):
...
Это объясняет, почему корабли **толкают** друг друга и **не могут** быть запушены: масса 32 vs 1 у пехоты.
...
**Вывод:** здания — массивные неподвижные блокеры с радиусом 0.35 (плюс отдельный footprint-mask у cell-grid из [building_mechanics.md](building_mechanics.md)). Юниты их объехать должны pathfinding'ом, локально CI лишь не даёт впереть в стену.
...
1. **Союзный → союзный**: чисто физика CI (mass/intersect/inertia), без событий. Юниты «слипаются» и плавно расталкиваются, передний по направлению пушит заднего.
