...
economics and calculations in the [economy guide](../../docs_en/reference/01_economy/README.md).
...
> **Related documents:**
>
> - [Recon: determinism of loot and combat (RNG audit)](../engine/determinism_audit.md) - RNG sites in hot
...
> - [Recon: server architecture and network synchronization](../engine/server_sync_architecture.md) —
> server-authoritative architecture C3 (important for multiplayer measurements).
> - [map generation](../../docs_en/recon/world/map/map_generation_pipeline.md) — timeline
...
> [Recon: determinism of loot and combat (RNG audit)](../engine/determinism_audit.md) §3). Scatter between
...
[map generation](../../docs_en/recon/world/map/map_generation_pipeline.md). Below is only
...
|---:|---|---|
| 1 | Full list of efficiency upgrades for all 21 nations | Use `parser/simulate_upgrades.py`, which expands `SetUpgStruct` and iterates over `case cid`. |
| 2 | Transport Ship (`ferry`) use for isolated forest islands | Test a route that includes embarkation, crossing, and disembarkation on a divided-island map. |
| 3 | Effect of `walkintervalfactor` on walking | Compare animation speed and physical movement at several values. |

**Level B (formulas):** §3–§7 contain the required values. Physical Peasant
speed is already included as `TrackPointMoveStep × 32 = 1.20` tiles per
game second.

**Level C (simulator):** additionally requires item 1, the complete list of
efficiency upgrades.
