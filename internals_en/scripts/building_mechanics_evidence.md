...
<a id="технический-разбор-зданий"></a>
# Technical Evidence for Building Mechanics

[← Scripts and Scenarios](structure.md)

[Reader-facing building article](../../docs_en/recon/world/economy/building_mechanics.md)
...
- **Capture radius:** four cells (see [building capture](../../docs_en/recon/world/economy/capture_mechanics.md)).
...
[construction-time report](../../docs_en/reports/economy/construction_times.md)
...
[builder-position report](../../docs_en/reports/economy/builder_slots.md) and
...
`OnTagStates` sets `bbuilt = True`. See [Walls and Gates](../../docs_en/recon/world/combat/walls_and_gates.md).
...
The listed price applies **to one segment**. A Wall line is created as a set
of separate objects, and the cost is charged when each foundation is placed.
...
upgrades. See
[Peasant gathering](../../docs_en/recon/world/economy/peasant_extraction.md#mines).
...
[known limitations](../project/known_issues.md).
...
**Eligibility:** unfinished buildings are checked regardless of
`bcapture`. After construction is complete, checks continue only when
`objprop.bcapture = True` and the match rules permit building capture.
This is why an unfinished Tower can be captured while a completed one
cannot.
...
- **Capture is instantaneous.** An in-game test on 29 April 2026
  ...
  `capturer_in_radius && no_protector_in_radius` changes ownership.

**Completed buildings that can be captured:** Mills, Markets,
Storehouses, Mines, Town Halls, Housing, Academies, Artillery Depots,
and Blacksmiths. The [capture article](../../docs_en/recon/world/economy/capture_mechanics.md#buildings)
lists their internal IDs.
...
[^21]: Capture checks for unfinished and completed buildings —
       `units/building.inc/nothing.inc:298-326`; capturer and protector
       searches — `lib/miscext.script:1018-1070`.
