...
> **Buildtime в game-time.** Для зданий движок хранит `objbase.buildtime = frames × (1/32) × gc_buildtime_modifier`, где `gc_buildtime_modifier = 10` ([`misc.script:478-482`](C:/Program%20Files%20(x86)/Steam/steamapps/common/Cossacks%203/data/scripts/lib/misc.script#L478)). Для юнитов модификатор = 1. Это объясняет, почему «реальное» время постройки здания ≈ frames × 10/32, а не frames/32. В нашем `output/data.json` поле `building.buildtime_sec` уже учитывает ×10.
...
Полная таблица «время с N крестьян» по всем зданиям всех наций — в [`output/reports/construction_times.md`](../output/reports/construction_times.md) (генератор: [`compute/compute_construction_times.py`](../compute/compute_construction_times.py)).
