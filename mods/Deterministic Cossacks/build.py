...
PATCHES = [
    ...
    },
    {
        ...
        "replacement": [
            ...
            "                        damage := 250;",
            "                        TObj(pobj).hp := 1; // partial cost: hp dropped to 1 unconditionally (mod by Deterministic Cossacks)",
            "                        bMusketSuicide := True;",
            "                        var trgUsageSuicide : Integer = _unit_GetUsage(trgHnd);",
            "                        if (trgUsageSuicide = gc_obj_usage_shooter) or (trgUsageSuicide = gc_obj_usage_horseshooter) then",
            "                        TObj(pobj).attackdelay := MaxFloat(TObj(pobj).attackdelay, 100.0); // death marker only vs shooter targets (mod by Deterministic Cossacks)",
            "                        end;",
            "                        end;",
            # other modifiers only apply on normal shots
            "                        if (not bMusketSuicide) then begin",
            # all damage modifiers below fire only when attacker and victim are stat-identical:
            # same usage AND same maxhp AND same weapon[0].damage. Captures national variants
            # that share stats (e.g. European musketeers) and excludes truly different units
            # (pikeman 90hp vs roundshier 100hp; Polish vs Turkish pikeman with different maxhp).
            "                        var trgUsageDmg : Integer = _unit_GetUsage(trgHnd);",
            ...
            "                        var pobjbaseTrgStat : Pointer = gPlayer[TObj(pobj2).pl].objbase[TObj(pobj2).cid][TObj(pobj2).id];",
            "                        var bSameStats : Boolean = (attUsageDmg = trgUsageDmg) and (attMaxHpF = TObjBase(pobjbaseTrgStat).maxhp) and (TObjBase(pobjbaseAttF).weapon[0].damage = TObjBase(pobjbaseTrgStat).weapon[0].damage);",
            "                        if (bSameStats) then begin",
            # pile-on penalty: 2nd attacker on same target deals 75%, 3rd 50%, 4th+ 30%
            ...
            # outnumbered bonus: +50% per enemy currently attacking the attacker
            ...
            "                        if (attOnMe > 5) then attOnMe := 5; // cap at 5 attackers (+250% max)",
            ...
            "                        end; // bSameStats (usage + maxhp + weapon[0].damage match)",
    ...
    },
    {
        ...
        "replacement": [
            ...
            "                              if (TObj(pobj).hp < 3) then TObj(pobj).attackdelay := MaxFloat(TObj(pobj).attackdelay, 100.0); // same-cat-death marker for retarget gate",
            "                              end;",
            "                              end; // bKillInfHorse",
            "                           end;",
        ],
    ...
    #     unit.script so the unit genuinely idles before it can join another fight.
    {
        ...
        "replacement": [
            ...
            "            if bGateInfHorse and (TObj(pobj).hp > 0) and (TObj(pobj).hp < 3) and (TObj(pobj).attackdelay > 50.0) then begin",
            "            TObj(pobj).hp := 0;",
    ...
    },
    {
        ...
        "replacement": [
            ...
            "                        if (trgMaxHp2>0) and (TObj(pTrgObj2).hp*2<trgMaxHp2) then",
    ...
    },
    {
        ...
        "replacement": [
            ...
            "                           if (trgMaxHp>0) and (TObj(pTrgObj).hp*2<trgMaxHp) then",
            "                           relativeDist := relativeDist*2.5;",
            "                        end;",
            "                        end else",
    ...
    },
    {
        ...
        "replacement": [
            ...
            "                           if (trgMaxHpR>0) and (TObj(pTrgObjR).hp*2<trgMaxHpR) then",
            "                           relativeDist := relativeDist*2.5;",
            "                        end;",
            "                     end else",
