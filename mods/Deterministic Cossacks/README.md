...
**Stock:** 5% шанс → `+floor(uniqrnd×500)` урона. Среднее «сырого» бонуса 250,
но из-за overkill эффективный бонус упирается в HP цели.
`E[min(uniform[0,500], H)] = H − H²/1000` для `H ≤ 500`.

**Мод:** всегда срабатывает для `bCanHeadShot` → детерминированный бонус,
рассчитанный по max HP цели:

`bonus = floor((H − H²/1000) × 0.05)`, где `H = clamp(maxhp, 0, 500)`

| maxhp цели | бонус |
|---|---|
| 50 (ранний пикинер) | +2 |
| 80 (типичный пехотинец) | +3 |
| 100 (тяжёлая пехота) | +4 |
| 200 (рейтары) | +8 |
| 500+ (тяжёлые юниты) | +12 |

Дисперсия и overkill устранены, эффективный DPS совпадает со стоковым.

**Suicidal shot (только мушкетёры, weapkind=bullet):** 5% шанс выстрела
по **любой цели** наносит **250 урона** и снижает hp мушкетёра до **1**.
Полный суицид (мгновенная смерть в retarget window через маркер) срабатывает
**только если цель — стрелок** (`shooter` или `horseshooter`); по
рукопашникам/лучникам мушкетёр просто остаётся на 1 hp без авто-смерти.
Обходит все прочие модификаторы урона.

### 2.2 Модификаторы урона (miscext2.script:437)

Применяются к удару **пехоты и лошадей**, и **только когда атакующий и жертва
стат-идентичны** — совпадают одновременно `usage` + `maxhp` + `weapon[0].damage`.
Это даёт «балансировку только в равном бою»: национальные варианты одного юнита
со скопированными статами (например, многие европейские мушкетёры) проходят
проверку, а реально разные юниты — нет.

Примеры:

| Пара | Проверка | Результат |
|---|---|---|
| Российский мушкетёр vs польский мушкетёр (статы скопированы) | usage/maxhp/damage совпадают | модификаторы применяются |
| Пикинёр (90 hp) vs рондашир (100 hp) | разные maxhp | модификаторы не применяются |
| Российский пикинёр (90 hp) vs турецкий пикинёр (95 hp) | разные maxhp | модификаторы не применяются |
| Мушкетёр vs пикинёр | разные usage и stat | модификаторы не применяются |

В кросс-стат стычках урон идёт по стоковой формуле (база − броня + squad-бонусы).
...
- **Fatigue:** применяется только при убийстве юнита **той же боевой категории**.
  ...
  от убийства рукопашников; стрелки (`shooter`, `archer`, `horseshooter`) — от
  убийства стрелков. Размер: пехота — `-15% maxhp`; лошади — `-5% maxhp`.
  Minimum 1 hp — усталость не убивает напрямую.
- **Near-death kill:** срабатывает только при **same-category** убийстве, когда
  fatigue довела `hp < 3` — или при суицидальном выстреле мушкетёра. Технически:
  same-cat kill ставит маркер `attackdelay := 100.0`, retarget gate проверяет
  `attackdelay > 50.0` и убивает (см. п. 2.5). Кросс-категория и обычный урон
  до `hp < 3` смерти не вызывают.

Цепочка усталости (same-category): пехотинец (maxhp ~80) теряет 12 hp за
убийство другого рукопашника. После ~6 таких убийств hp опускается до 1.
При следующем убийстве hp=1 < 3 → смерть в retarget gate. Стрелки аналогично
устают от убийства стрелков.
...
Дополнительно: цели с `hp < 50% maxhp` получают ещё ×2.5 к дистанции —
юниты обходят раненых и переключаются на свежих. Это касается и умирающих
(`hp < 3`) — они тоже депрайорити.

### 2.5 Retarget gate (unit.script:8400)

Применяется только к **пехоте и лошадям**.

- `attackdelay > 0` — юнит не ищет новую цель (принудительный idle).
- `hp < 3` + `attackdelay > 50.0` — смерть (`hp := 0` + `gc_statetag_essential_death`).
  Маркер `attackdelay > 50` ставится только в kill block при same-cat fatigue,
  доведшей hp до < 3, или в suicide shot — поэтому срабатывает строго в same-cat
  логике.
