...
> covers visibility and shared vision; [Unit Orders](../world/combat/unit_commands.md)
> explains the commands that units can follow.
...
<a id="что-важно-игроку"></a>
## What the Player Needs to Know

<a id="выделение-и-камера"></a>
### Selection and Camera

A click selects the object under the cursor; a selection box gathers all
eligible objects inside it. Selection shortcuts can find all Peasants, an
idle Peasant, a Mine with free capacity, ships, or units of one type. Ten
control groups are numbered 0 through 9. A selection can be stored, extended,
and recalled later.

The mouse wheel controls zoom by default. The camera can also jump to selected
units, the starting position, or the latest alert. Sound volume is normally
measured from the camera, so the player mainly hears activity near the
current view.

<a id="звуки-и-предупреждения"></a>
### Sound and Alerts

Audibility does not depend on fog of war. A hidden opponent can reveal nearby
activity by firing within hearing range of the camera. Attack and capture
alerts concern the local player's objects and appear only when the event is
off-screen; repeat alerts are suppressed for several game seconds. The alert
hotkey moves the camera to the event.

<a id="горячие-клавиши-скорость-и-пауза"></a>
### Hotkeys, Speed, and Pause

Configurable shortcuts depend on context. The same key may place a building
while the construction menu is open and issue a different order to selected
units. System shortcuts for camera movement, pause, chat, screenshots, and
control groups are reserved. Match speed changes the number of simulation
steps per real second. In multiplayer, each player may pause four times for
up to 120 seconds per pause.

<a id="техническое-приложение"></a>
## Technical Appendix

The sections below record interface functions, the hotkey configuration
format, and reserved combinations. They document implementation details; the
player-facing behavior is summarized above.
...
Selection behavior is configured through
...
The combinations for assigning and recalling **control groups** are listed
below. The native executable performs these operations; the script layer has
no separate control-group interface.

---

<a id="2-ввод-мышь-скролл-клавиатура"></a>
<a id="2-ввод-мышь-колесо-клавиатура"></a>
### 2. Input: mouse, wheel, keyboard
...
| `GetGUIPreviousMouseCoord(var ax, ay)` | Previous cursor position, used to calculate movement. |
...
<a id="23-колесо-мыши"></a>
#### 2.3. Mouse wheel

`GetGUIEventMouseWheelDelta()` returns the most recent wheel delta
(positive = up, negative = down). By
default, it controls camera zoom:
- `SetCameraControlMouseWheelDistance(True)` — wheel changes
  distance.
- `SetCameraControlMouseWheelRotate(True)` — wheel rotates the camera.
...
The engine handles hotkeys through interface states such as `OnKey_F1` and
`OnKey_Ctrl_A`. Scripts do not redefine bindings directly; user bindings
are stored in the profile and changed through the game's settings.
...
| `GUIShowCursor(show)` | Show or hide the cursor. |
...
In an ordinary random-map match, the listener follows the camera. The player therefore
...
**These functions do not check fog of war.**

<a id="52-что-это-значит"></a>
#### 5.2. What this means in play

- An enemy unit hidden by the player's fog of war can still be **heard** when it
...
- Sound can therefore reveal nearby enemy activity before one of the
  player's units reveals the area.

<a id="53-почему-так-сделано"></a>
<a id="53-как-устроено-разделение"></a>
#### 5.3. How the separation works

The systems use different inputs:

1. **The listener follows the camera** in the standard mode, whereas fog of
   war is calculated separately for each player.
2. Audibility depends on distance from the listener and does not require
   line of sight to the source.

See also [Vision and Fog of War](../world/combat/vision_and_fow.md)
for details on the visibility system.
...
The player can jump the camera to the event with the alert hotkey assigned
...
- **A Priest is the acting unit.** Priests have the `bpriest` flag and heal
  rather than attack. `_misc_DoDamage` skips the alert when the acting unit
  is a Priest.
...
| `squad` | Formation commands | `squad\|fill`, `squad\|disband`, `squad\|rank` (line), `squad\|column`, `squad\|square` |
...
| `Z` | `select\|unitsofsametype` (repeat to select the next one) |
...
| Key | Reserved for |
|---|---|
| `LButton`, `RButton`, `MButton` | Mouse buttons—selection, orders, and camera dragging. |
...
| **`NUM0`–`NUM9`** | Numpad digits reserved for control groups. |
...
| **`Shift+Alt+0` … `Shift+Alt+9`** | Reserved additional control-group mode. |
...
<a id="83-классы-групп-в-исполняемом-файле"></a>
#### 8.3. Control-group classes in the executable

The executable's runtime type information (RTTI) contains `TXGroup4` and
`TXGroupSelectionViewer`; the latter draws **green highlights** around units
in the active control group. The script layer has no separate numeric-key
binding for groups, so the native executable handles these commands.

<a id="9-игровой-темп-и-пауза"></a>
### 9. Game speed and pause
...
| `fast` (2) | **14 ticks/sec** (default in a random-map match) |
...
Native functions for checking keys [^2]:
...
Gameplay scripts process input through `SetGUIEventStateOn*` interface
states. The native key-query functions remain available through the scripting
interface, but the shipped gameplay logic does not call them. The active
interface state already includes modifiers such as Shift.
...
[^2]: Native functions `IsKeyDown`, `IsKeyDownByName`, `KeyPressed` —
      see [`derived/dws_native_signatures.json`](../../../derived/dws_native_signatures.json).
      Hotkey config — `data/game/var/hotkeys.cfg`. Reserved-key lists —
