...
- При смерти офицера отряд **рассыпается** и теряет все формационные бонусы — это и есть «потеря ауры», которую ощущают игроки.
...
Огненные стрелы (`barrow`) работают иначе: если в области не больше 300 юнитов — урон получают **все** в радиусе; иначе только те, кто строго внутри `r`.
