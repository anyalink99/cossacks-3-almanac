...
def name_cell_short(item: dict) -> str:
    """Compact name cell for compare/ tables: `**EN name** ` `sid` `."""
    name_en = (item.get("name_en") or "").strip()
    sid = item.get("sid", "")
    if name_en:
        return f"**{name_en}** `{sid}`"
    return f"`{sid}`"
...
def write_readme(data: dict) -> None:
    ...
    A("```")
    A("**Минимум 1 хп**. Headshot = 5% шанс на каждый bullet/arrow выстрел против любого "
      "юнита (кроме light-cavalry-в-движении) даёт **до +499** бонусного damage. См. подробности "
      "в [02_combat.md → Headshot](02_combat.md#headshot-критический-удар--главная-скрытая-механика). "
    ...
    A("")
    A("## Расхождения с заметками промпта\n")
...
def write_economy(data: dict) -> None:
    ...
    A("| Уровень | +Воркеров | F | G | Накопительно |")
    A("|---:|---:|---:|---:|---:|")
    cumulative = 5
...
def write_combat(data: dict) -> None:
    ...
    A("[← Index](README.md)\n")
    A("## Содержание\n")
    A("- [Damage formula (полная)](#damage-formula)")
    A("- [Headshot — критический удар](#headshot-критический-удар--главная-скрытая-механика)")
    A("- [Формационные бонусы (LINE/SQUARE/KARE)](#формационные-бонусы)")
    A("- [Dispersion — рассеяние выстрелов](#dispersion--почему-выстрелы-промахиваются)")
    ...
    A("- [RunAway — авто-кайтинг стрелков](#runaway--авто-кайтинг-стрелков)")
    A("- [Friendly fire — дружественный огонь](#friendly-fire--дружественный-огонь)")
    A("- [Multi-weapon switching (картечь, штык, огненные стрелы)](#multi-weapon-switching--переключение-оружия-по-дистанции)")
    A("- [Movement penalty к дальности (standtime)](#movement-penalty--штраф-к-дальности-при-движении)")
    A("- [Idle bonus к дальности (addradius)](#idle-bonus--бонус-к-дальности-в-покое)")
    A("- [Capture mechanic — захват юнитов](#capture-mechanic--захват-юнитов)")
    A("- [Priest healing — лечение священниками](#priest-healing--лечение-священниками)")
    A("- [Shield /3 при недостроенном здании](#shield-3-при-недостроенном-здании)")
    A("- [AI aggression flip от damage](#ai-aggression-flip--ai-переходит-в-атаку-от-одного-удара)")
    A("- [Officers — миф о combat-aura](#officers--миф-о-combat-aura)")
    A("- [Чего НЕТ в игре](#чего-нет-в-игре-confirmed-absent)")
    A("- [Свойства damage formula](#свойства-damage-formula)")
    A("- [Типы оружия](#типы-оружия-gc_obj_weapon_kind_)")
    A("- [Скорости юнитов](#скорости-юнитов)")
    A("- [Офицеры и формации](#офицеры-и-формации)")
    A("- [Counter matrix (приближённый TTK)](#counter-matrix-приближённый-ttk)")
    A("- [Upgrade × stat cross-reference](#upgrade--stat-cross-reference)")
    ...
    A("### Headshot (критический удар) — главная скрытая механика\n")
    A("**5% шанс на каждый выстрел** добавить `floor(uniqrnd × 500)` damage. "
    ...
    A("- `uniqrnd` **зафиксирован при спавне** юнита-стрелка → у каждого индивидуального "
      "мушкетёра свой headshot damage. В отряде из 36 мушкетёров будут «снайперы» "
      "(uniqrnd≈0.9 → +450 dmg) и «мазилы» (uniqrnd≈0.05 → +25 dmg).")
    A("- Среднее ожидаемое: `0.05 × 250 = 12.5` дополнительного damage на выстрел "
      "(выровненный по случайным uniqrnd ~0.5).")
    A("")
    A("**Пример:** мушкетёр стреляет в Reiter (282 hp). Обычный damage — 6 hp. "
      "На случайном выстреле (5%) случается headshot, и тот же мушкетёр (uniqrnd≈0.252) "
    ...
    A("- Стрелковые отряды против Heavy Cavalry/Light Infantry статистически окупаются "
      "сильно лучше чем damage formula показывает.")
    A("- Light Cavalry **в движении** иммунна к headshot → главный counter к стрелковому отряду.")
    A("- C1 имел 4% шанс instant-kill (комментарий в коде); в C3 механика была ребалансирована "
    ...
    A("Юниты в строю получают **+damage / +shield** к каждому выстрелу/попаданию. "
    ...
    A("")
    A("**Ключевое:** мушкетёр с base damage 6 → **в LINE-формации hold-mode наносит 13 damage** "
      "за выстрел (6 + 7 hold). И принимает -7 от каждого входящего попадания (поверх protection).\n")
    A("**Что это значит для боя:**")
    A("- Стрелковые отряды в **hold-mode на формации** = **+117% damage** (с 6 до 13).")
    A("- Без формации (рассыпная) — никаких бонусов.")
    A("- Кавалерийские формации (treugol'nik, klin) дают только +1/+1 — формация для них не главное.")
    A("- **firearrow** (зажигательная стрела) НЕ получает squad damage bonus.")
    A("")
    ...
    A("Взрывы (cannon, mortar, gun, grenade) попадают по всем юнитам в радиусе `r`, **но "
      "только первые N получают damage**:\n")
    A("```")
    ...
    A("Стрелы с зажигалкой (`barrow`) имеют другую логику: damage всем в радиусе если "
    ...
    A("Если ranged-юнит стоит на возвышенности (Y > 0), его **search distance** увеличивается:\n")
    A("```")
    ...
    A("- Бонус **только к detection range** (видят дальше / start firing раньше).")
    A("- Сам выстрел (`radiusmax`) технически тот же, но если враг ещё не в радиусе атаки, "
      ...
      "**мушкетёры с холма стреляют по атакующим раньше** = больше выстрелов до melee.")
    A("- НЕ работает на melee юнитов (pikemen в milee remain melee).")
    A("- Холмы на карте создаются по `relief` map gen параметру (Highlands map даёт максимум гор).")
    A("")
    A("### Score за убийство\n")
    ...
    A("- **Без standground** мушкетёр (radiusmax≈9 t) обнаруживает врага только когда тот "
      ...
      "и сразу melee. Это объясняет почему стрелковые юниты «стоят и не стреляют» по дальним целям.")
    A("- **С standground** обнаружение работает на полную `searchradius` (1500-2400 px ≈ 28-45 t). "
      "Мушкетёр стреляет по любому врагу в радиусе обзора — успевает 5-10 выстрелов до melee.")
    A("- **Standground также отключает RunAway** (см. ниже): юнит держит позицию, не пытается отступать.")
    A("- **Move-приказы стирают standground**: если юниту приказали идти куда-то, он не стреляет даже "
    ...
    A("- **При продвижении** (`bstandground=False`) стрелки кайтят (RunAway) — это иногда плюс "
      "(не дают подойти), иногда минус (тормозят свой же штурм).")
    A("- **Артиллерию лучше use attackpoint** (Ctrl+ЛКМ) — `bartprepare` правильно настраивает "
      "режимы. Если просто переместить пушку и ждать — она в movement-режиме НЕ откроет огонь.")
    A("")
    A("### RunAway — авто-кайтинг стрелков\n")
    ...
    A("\n**Эффект:** стрелок отступает на 3.5 тайла от врага, пытаясь восстановить дистанцию для выстрела. "
      "Это создаёт классический **kiting-pattern** мушкетёров: подошёл → стрельнул → отступил → стрельнул.\n")
    A("**Когда RunAway ВЫКЛЮЧЕН:**\n")
    A("- В `standground` (явный приказ держать позицию).")
    A("- AI-противник на hard+ — продолжает стрелять до самого melee, не отступает (опасный нюанс).")
    A("- На AI-hard+ человеческий игрок: AI кайтит как обычно, но human player на hard+ тоже не получает "
      "RunAway-помощь.")
    A("\n**Стратегические выводы:**\n")
    A("- Для отступательной тактики (kite-and-shoot) — **сними standground** и работай "
      "по уязвимой кавалерии/пехоте.")
    A("- Для удержания позиции (например, на холме) — **standground обязателен**, иначе мушкетёры "
      "разбегутся при подходе melee.")
    A("- Лёгкая кавалерия может **догнать кайтящих мушкетёров** (fasthorse=96 vs default=32 → ~3× быстрее).\n")
    ...
    A("**Дружественный огонь ВКЛЮЧЁН для большинства снарядов.** В функции `_misc_DoDamage` "
      "**нет проверки на team/owner** — урон применяется к любому объекту, попавшему под траекторию.\n")
    ...
    A("- AoE взрывы (картечь, ядра, бомбы) — поражают **всё в радиусе**, включая своих "
      "(`_misc_DoRoundDamage` без team-фильтра).")
    ...
    A("- **Башни без line-of-sight** через здания не выстрелят (если их weapon `bcheckfriendonline=True`), "
      "но снаряд при выстреле уже не различает свой/чужой.\n")
    A("### Multi-weapon switching — переключение оружия по дистанции\n")
    ...
    A("**Главные пары:**\n")
    A("**1. Cannon (пушка) — ядро vs картечь:**\n")
    ...
    A("\n**Это значит:** если пехота подошла на ~8 тайлов к пушке — она автоматически переходит в "
      ...
      "см. AoE damage cap).\n")
    A("**2. Musketeer 18c — пуля vs штык (bayonet):**\n")
    A("| Слот | Тип | dmg | Pause | Range (px) | Когда стреляет |")
    A("|---|---|---:|---:|---|---|")
    ...
    A("\n**Стратегический смысл:** мушкетёр после выстрела не беспомощен в melee — у него **штык** "
      "с pause=0 (бьёт каждый animation cycle). Атаковать перезаряжающихся мушкетёров кавалерией = "
      "получить штыковой бой. Прусские мушкетёры (dmg штыка 10) сильнее в melee чем баварские (5).\n")
    A("**Прокачки штыка** идут отдельно от прокачек пули — `bla.musketeer18.1.X` качает урон bullet, "
      "штык остаётся базовый.\n")
    A("**3. Archer — обычная стрела vs огненная (firearrow):**\n")
    ...
    A("\n**Огненные стрелы — главное оружие лучников против построек.** Урон 150 (×10 от обычной), "
      "но: ниже скорострельность (-40%), хуже точность (+14% дисп.), не получают **squad damage bonus** "
      "(см. секцию Headshot/Damage formula). Игра автоматически переключает лучника на OSTRELA когда "
      "цель — здание/wood/палисад. **Лучники — лучший анти-билдинг ranged юнит** (если успевают подойти).\n")
    A("**4. Другие multi-weapon юниты** (поищи `weapon[1]` в их sid):\n")
    A("- **Janissary (jannisary)** — пуля + сабля melee.")
    A("- **Strelet** — пищаль + бердыш melee.")
    A("- **Jaeger / dragoon** — пуля + сабля.")
    A("- **Cavalry shooters** (drabant, mounted strelet) — пуля + сабля верхом.")
    A("\nВо всех случаях работа одинакова: близко = melee weapon, далеко = ranged. **Разница cooldown'ов:** "
      "например мушкетёр перезаряжается 150 frames пулю и 0 штык — значит **сразу после выстрела** "
      "может ткнуть штыком если враг близко (а потом перезарядка пули продолжится в фоне).\n")
    A("### Movement penalty — штраф к дальности при движении\n")
    ...
    A("- В сочетании с RunAway создаёт **kite-pause-shoot**: мушкетёр отбежал на 3.5 t, ждёт "
      "0.25 сек чтобы вернуть полную дальность, стреляет, и снова кайтит.\n")
    A("### Idle bonus — бонус к дальности в покое\n")
    ...
    A("### Capture mechanic — захват юнитов\n")
    ...
    A("        attacker.SetOrderTrg(target)  # лочится на захват")
    A("```")
    ...
    A("- **5% chance per tick** — захват не мгновенный, нужно несколько секунд возле цели. "
      "За 5-7 сек шанс ~25-30% завершить попытку.\n")
    A("### Priest healing — лечение священниками\n")
    ...
    A("```")
    A("**Heal pause = 0** — лечат каждый animation cycle (~0.7 сек), пока цель не full HP.\n")
    A("| Юнит | heal/удар | range (px / tiles) | Где доступен |")
    A("|---|---:|---|---|")
    ...
    A("- **Несколько священников лечат одну цель параллельно** — рейтер с 282 HP лечится 4 priest'ами "
      "= +80 HP/cycle = ~115 HP/sec. Можно держать тяжёлую кавалерию вечно.")
    A("- **Mullah имеет самую большую дальность** (9.4 t) — лечит из второй линии, недосягаем для melee.")
    ...
    A("При расчёте damage: если здание **ещё строится** (`bbuilt=False`), его shield делится на 3:\n")
    A("```")
    ...
    A("**Стратегический смысл:** **сноси здания пока они строятся**. Например, башня на стройке имеет "
      ...
      "(rush на возводимое здание противника) гораздо эффективнее чем атака готового.\n")
    A("Касается ТОЛЬКО зданий — юниты не имеют состояния «строится».\n")
    A("### AI aggression flip — AI переходит в атаку от одного удара\n")
    ...
    A("Любой не-артиллерийский юнит AI, **получивший damage**, флипает свой отряд (`squad`) в "
      "`fAgressive=True` и обновляет `fLastBattleTime`.\n")
    A("**Эффект:** один поражающий выстрел/удар по AI-отряду переводит его в боевой режим. "
      "AI начинает контратаковать, преследовать, искать врага активно.\n")
    A("**Стратегические выводы:**\n")
    A("- **Поклёвывание AI** (один лучник в крестьянина) **триггерит реакцию всего AI-отряда**. "
      "Может быть полезно: отвлекаешь лучником, основная сила атакует с другой стороны.")
    A("- AI на artillery (пушки/мортиры) — НЕ флипает (особый case в коде).")
    A("- Если хочешь скрытно собрать ресурсы рядом с AI — **не атакуй вообще**, иначе вся армия "
      "зашевелится.\n")
    A("### Officers — миф о combat-aura\n")
    ...
    A("**В игре НЕТ персональной aura-бонуса от офицера.** Офицеры/барабанщики занимают слоты "
    ...
    A("- При смерти офицера отряд **рассыпается** → теряет все формационные бонусы (это и есть "
      "«потеря aura» которую ощущают игроки).")
    A("\n**Стратегический вывод:** **убийство офицера = убийство всех бонусов отряда**. "
      ...
      "офицера это всё пропадает мгновенно. **Офицеры — снайперский target №1.**\n")
    A("### Чего НЕТ в игре (confirmed absent)\n")
    A("После проверки `unit.script`, `weapon.script`, `miscext2.script`, `player.script`:\n")
    A("- **НЕТ cavalry charge bonus.** Кавалерия не получает бонусного урона на разгоне или при "
      "первом ударе. Поиск `bcharging`/`firsthit`/`chargebonus` ничего не находит. "
      "Урон кавалерии = базовый damage оружия.")
    A("- **НЕТ anti-horse damage type.** Pikemen не имеют умножителя «×N против кавалерии». "
      "Эффективность пикинёра против рейтара — это просто `weapon.damage(pike)` против "
      "`target.protection[pike]` где у кавалерии этот protection обычно низкий.")
    A("- **НЕТ drummer aura.** Барабанщик — просто слот в формации (формальное наполнение). "
      "Не даёт +damage, +speed, +morale.")
    A("- **НЕТ grenadier-special arc.** Гранатомёт использует обычный AoE-pipeline (cannonball-kind с "
      "explosion radius). Никакого specials у траектории нет.")
    A("- **НЕТ stealth/invisibility.** Все юниты видны если в vision'е игрока. Нет флага `bstealth`.")
    A("\n**Что это меняет:** **формация — единственный способ умножить урон**. Никаких скрытых "
      "буффов от позиции (кроме high-ground / standground). Прокачки + формация + тип оружия "
      "vs тип брони — это вся боевая математика.\n")
    A("### Свойства damage formula\n")
    ...
    A("- Shield применяется ВСЕГДА (включая поверх protection). Танки с shield эффективнее тяжёлых protection.")
    A("- При постройке здания shield делится на 3.")
    A("- `firearrow` (зажигательная стрела лучников) НЕ получает squad damage bonus.\n")
    A("### Dispersion — почему выстрелы промахиваются\n")
    ...
    A("\nПример: мушкетёр (disp=3.75) на 15 t → maxdisp=1.50, окно ±1.50 = 3.00 → шанс попасть "
      "в 1×1 цель ≈ 1/3 = **~33%** одним выстрелом. Это означает что **counter matrix TTK ниже "
      "реального в 3 раза для дальних bullet/arrow**.\n")
    ...
    A("|---:|---|---|")
    A("| 1 | Headshot bonus | `+floor(uniqrnd × 500)` дополнительного damage при крите |")
    A("| 2 | Эффективная max range | `radiusmax -= uniqrnd × 3` тайла. Каждый стрелок шарашит чуть на разную дистанцию (асинхронные залпы) |")
    ...
    A("\nЭффекты — это **trade-off** built into каждый юнит: высокий uniqrnd → большие криты, "
    ...
    A("| `sword` | Меч/сабля | Light infantry, swordsmen, кавалерия в melee |")
    ...
    A("Базовые `gc_obj_speed_*` из `dmscript.global:603-620`. **Абстрактные единицы** (не tiles/sec). "
      ...
      "и game speed. Для перевода нужен empirical test.\n")
    ...
    # Counter matrix
    A("## Counter matrix (приближённый TTK)\n")
    ...
    A("Расчёт: для атакующего берём **репрезентативного юнита класса** (медианный по damage); "
      "для защитника — медианный по HP. Применяется damage formula:\n")
    ...
    A("⚠ Цифры ориентировочные. Реальный TTK будет выше из-за: дороги к цели, "
      "подготовки выстрела (`bartprepare`), формационных бонусов (squad shield, формация LINE/SQUARE/KARE), "
    ...
    A("**Median представители классов** (использованы для расчёта):\n")
    A("| Класс | Atk-репрезентант | dmg | reload (s) | kind | Def-репрезентант | HP | shield |")
    A("|---|---|---:|---:|---|---|---:|---:|")
    ...
    A("")
    A(f"### Counter matrix — TTK в игр-сек\n")
    ...
    for atk_cls in BATTLE_CLASSES:
        ...
        A(row)
    A("")
    A("**Чтение:** жирным — быстро убивает (TTK<1 sec), курсивом — почти не убивает (TTK>100 sec).\n")
    # Now: miss-adjusted TTK for ranged shooters
    A("### Counter matrix с поправкой на промахи (для bullet/arrow)\n")
    A("Для bullet/arrow атакующих TTK выше из-за **dispersion**: на дистанции 15 t мушкетёр "
      "попадает только ~33% выстрелов (см. секцию Dispersion). Здесь TTK умножен на коэффициент "
      "промахов:\n")
    A("```")
    ...
    A("```")
    A("Считаю на дистанции 12 t (типичная дистанция боя стрелков), с baseline dispersion=200 px/3.75 t. "
      "Для melee (cannon/mortar в melee?) дистанция 6 t как fallback. Ниже — relative TTK для "
      "стрелков (только rows: Mus17, Mus18, Arch, Drag, LtCav (если стрельба)):\n")
    ...
    A("- В **hold-mode формации** damage +7 (с 6 до 13) → TTK падает в ~2 раза. "
      "С учётом промахов — формация компенсирует dispersion примерно как раз.")
    ...
    A("")
    A("**Примеры выводов** (на default protection данных):")
    A("- **Cannons / Mortars** против пехоты — TTK <1 (полный one-shot за выстрел).")
    A("- **Pikemen** против Heavy Cavalry — TTK высокий, потому что cavalry имеет prot_pike. "
      "Но в формации pikeman даёт сильно больше DPS.")
    A("- **Musketeers** против пехоты с prot_bullet=4-6 — TTK ~10-15 sec, реалистично.")
    A("- **Light Cavalry** vs **Cannons** — низкий TTK (cannon без брони, легко убивается).\n")
    A("## Upgrade × stat cross-reference\n")
    A("Какой апгрейд на что влияет. Сводка по `itype` (расшифровано в "
      "[05_upgrades.md](05_upgrades.md)). Цены даны для baseline нации (отличаются по "
      "нациям — см. [05_upgrades.md](05_upgrades.md)).\n")
    A("**Подсказки по нотации:** `aca.X` = academy.X, `bla.<unit>.1.X` = blacksmith damage X-th level "
      "for unit. `mil.X` = mill.X. Названия — из локали (en).\n")
    ...
    A("| Apgrade | Effect | val | Cost (F/W/S/G/I/C) | Что улучшает |")
    A("|---|---|---:|---|---|")
    for u in aca_ups:
    ...
    A("- **+firearm damage %**: `aca.12` (+10) + `aca.13` (+10) + `aca.14` (+15) + `aca.15` (+25) = "
      "**+60% damage** для всех bullet/arrow юнитов.")
    A("- **+artillery range %**: `aca.16` (+5) + `aca.17` (+10) = **+15% range**.")
    A("- **+artillery accuracy %**: `aca.20` (-35%) + `aca.27` (-35%) = **-70% dispersion** "
      "(почти точные выстрелы).")
    A("- **+artillery durability %**: `aca.18` (+50%).")
    A("- **+firearm reload %**: `aca.31` (-30%) + `aca.33` (-30%) = **-60% reload time** "
      "(скорость стрельбы +250%). Применяется ко **ВСЕМ bullet-стрелкам** — мушкетёрам, "
    ...
    A("- **One-time effects**: `aca.21` (heal artillery), `aca.22` (geology — reveal mines), "
      "`aca.25` (Montgolfier — reveal map), `aca.26` (heal all units), `aca.30` (×10 ship build "
      "speed), `aca.32` (-50% musket cost).")
    A("- **Enable units**: `aca.6` (frigate), `aca.19` (multicannon), `aca.29` (battleship).")
    A("")
    A("### Per-unit апгрейды (blacksmith / barracks / stable)\n")
    A("Кузница (`bla`), бараки (`bar`/`ba2`), конюшня (`sta`) содержат **per-unit damage и protection** "
      "апгрейды (5 уровней + level 7 special). Формат sid: "
    ...
    A("- 5 уровней damage (`.1.1` … `.1.5`): +1, +2, +2, +1, +2 = **+8 damage**")
    A("- 5 уровней protection (`.2.1` … `.2.5`): +1, +1, +2, +1, +1 = **+6 protection** (pike, sword, arrow)")
    A("- Level 7 unique (`.1.6` / `.2.6`): +2 damage / +2 protection (rus override)")
    A("- **Полный стек: +10 damage / +8 protection** на топовом prepared Russian pikeman.")
    A("")
...
def write_buildings(data: dict) -> None:
    ...
    A("**[Per-nation здания](#per-nation-здания)**")
    for suf in ["cen", "hou", "bar", "ba2", "bla", "sta", "tem", "aca", "art", "dip"]:
        if by_suffix_pn.get(suf):
            ...
            A(f"  - [{label}](#{anchor})")
    A("**[Common здания (по кластерам)](#common-здания-по-кластерам)**")
    ...
    A("Сводка: для каждого типа зданий — параметры по всем нациям (где они есть). "
      "**Жирным** — отклонения от baseline (mode of column).\n")
    for suf in ["cen", "hou", "bar", "ba2", "bla", "sta", "tem", "aca", "art", "dip"]:
        ...
        A("| Здание | Нация | HP | Time (s) | cost% | F | W | S | G | I | C | farm | produces |")
        A("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
        for b in rows:
    ...
    for suf in ["mil", "sto", "mar", "por", "tow", "gol", "iro", "coa", "swa", "sga", "wga", "wwa"]:
        ...
        A("| Здание (cluster) | Нации | HP | Time (s) | cost% | F | W | S | G | I | C | Доп. |")
        A("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
        for sid in sorted(by_sid.keys()):
            ...
            extra = []
            if b["weapon_damage"]:
                extra.append(f"dmg {b['weapon_damage']}")
                if b["weapon_radiusmax"]:
                    extra.append(f"range {round(b['weapon_radiusmax']/53.3333, 1)}t")
            if b["consume"]:
                extra.append(f"upkeep {json.dumps(b['consume'])}")
            if b["produce"]:
                extra.append(f"produce {json.dumps(b['produce'])}")
            if b["peasantabsorber"]:
                extra.append(f"peasants {b['peasantabsorber']}")
    ...
    mine_ups = sorted([u for u in data["upgrades"]
                       if u["sid"].startswith("eurgol.") and u["nation"] == "aus"],
                       key=lambda x: x["sid"])
    A("| Уровень | +Воркеров | Food | Gold | Накопительно |")
    A("|---|---:|---:|---:|---:|")
    cum = 5
    for u in mine_ups:
...
def write_units(data: dict) -> None:
    ...
    A("# 04. Юниты\n")
    A("[← Index](README.md)\n")
    A("Все юниты сгруппированы по классу. Для side-by-side сравнения внутри класса см. "
    ...
    for cls in classes_with_content:
        ...
        anchor = heading_anchor(f"{cls} {len(by_sid)} вариантов")
        A(f"- [{cls}](#{anchor}) ({len(by_sid)} вариантов)")
    A("")
    ...
    for cls in classes_with_content:
        ...
        for u in units:
            by_sid[u["sid"]].append(u)
        A(f"## {cls} ({len(by_sid)} вариантов)\n")
        A("| Юнит | nations | HP | Time | F | G | I | dmg | rng (t) | reload | пика | меч | пуля | картечь | стрела | ядро |")
        A("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
        for sid in sorted(by_sid.keys()):
            entries = by_sid[sid]
            u = entries[0]
...
def write_upgrades(data: dict) -> None:
    ...
    A("Апгрейды сгруппированы по **месту** (academy/blacksmith/mill/stable/barracks/mine/tower/wall/...). "
      "Каждый апгрейд — `<nat><place>.<unit>.<itype>.<level>` (per-unit, в кузнице/конюшне/казарме), "
      "`<nat><place>.<level>` (один на нацию, в академии/мельнице/town hall), "
      "или `<cluster><place>.<level>` (общие для кластера: tower/wall/shipyard).\n")
    A("`itype_short` колонка декодирует raw `gc_upg_type_*` в человеческие термины.\n")
    ...
    PLACE_NAMES = {
        ...
        "mil": "Mill (food efficiency)",
        "art": "Artillery Depot (cannon upgrades)",
        "tem": "Cathedral (priest upgrades)",
    ...
    for place in PLACE_ORDER:
        if not by_place.get(place):
            continue
        if place == "mines":
            label = "Mine upgrades (eurgol/eurcoa/euriro)"
        else:
    ...
    A("## Mine upgrades (eurgol/eurcoa/euriro)\n")
    ...
    for place in [p for p in PLACE_ORDER if p != "mines"]:
        ...
        A("Каждая запись — одна нация. Если значение одинаковое для всех 21 нации, показано одной строкой "
          "с `nation=all`. Если есть per-nation override (через `_country_ModifyUpgrade`), показаны отдельно.\n")
        A("| Апгрейд | nations | itype | val | F | W | S | G | I | C | time |")
        A("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|")
...
def write_nations(data: dict) -> None:
    ...
    A("# Нации\n")
    A("[← Index](../README.md)\n")
    A("Полный список 21 играбельной нации. Каждая ссылка — отдельный cheatsheet "
      "с уникальными юнитами, зданиями, апгрейдами и зависимостями.\n")
    A("| ID | sid | EN | RU | cluster | peasant | unique units |")
    A("|---:|---|---|---|---|---|---|")
    ...
    for nat in PLAYABLE_NATIONS:
        ...
        A(f"- **Common cluster:** `{cluster}` (mill/sto/mar/tow используют `{cluster}+suffix`)")
        A(f"- **Peasant:** `{cluster_peasant.get(nat, '?')}`")
        A(f"- **Кластерная пехота:** `{cluster}` cluster")
        A("")
        # Uniques
        unique_units = sorted(sid for sid, nations in sid_to_nations.items()
                              if nations == {nat})
        ...
        if unique_units:
            A("| Юнит | usage | HP | dmg | reload | range (t) |")
            A("|---|---|---:|---:|---:|---:|")
            for sid in unique_units:
                ...
                if rows:
                    u = rows[0]
                    w0 = (u["weapons"] or [{}])[0]
                    A(f"| {name_cell_full(u)} | {u.get('usage_short') or '—'} "
            ...
            if suf not in suffix_baselines:
                ...
                suffix_baselines[suf] = compute_baselines(
                    ...
                    ["hp", "buildtime_sec", "costpercent", "food", "wood", "stone",
                     "gold", "iron", "coal", "farm"])
        A(f"### Per-nation ({len(per_nat_b)})\n")
        A("> **Жирным** — значения, отличающиеся от baseline (mode по всем нациям) "
          "для того же типа здания.\n")
        A("| Здание | HP | Time | cost% | F | W | S | G | I | C | farm | produces |")
        A("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
        for b in per_nat_b:
            suf = b["sid"][len(nat):]
            ...
            A(f"| {name_cell_full(b)} "
              ...
              f"| {prod_str or '—'} |")
        A("")
        A(f"### Common cluster ({len(common_b)})\n")
        A("| Здание | HP | Time | cost% | F | W | S | G | I | C | Доп. |")
        A("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
        for b in common_b:
            extra = []
            if b["weapon_damage"]: extra.append(f"dmg {b['weapon_damage']}")
            if b["consume"]: extra.append(f"upkeep {json.dumps(b['consume'])}")
            if b["produce"]: extra.append(f"produce {json.dumps(b['produce'])}")
            if b["peasantabsorber"]: extra.append(f"+{b['peasantabsorber']} workers")
            extra_str = "; ".join(extra) if extra else "—"
        ...
        for cls in class_order + sorted(c for c in by_class if c not in class_order):
            ...
            if not units:
                continue
            A(f"### {cls}\n")
            A("| Юнит | HP | Time | F | G | I | dmg | rng (t) | reload | uniqueness |")
            A("|---|---:|---:|---:|---:|---:|---:|---:|---:|---|")
...
def write_compare(data: dict) -> None:
    ...
    A("# Сравнения\n")
    A("[← Index](../README.md)\n")
    A("Side-by-side таблицы юнитов одного класса по всем нациям. Помогает выбирать пика-vs-пика, "
      "стрельца-vs-стрельца и т.д.\n")
    ...
    def write_unit_compare(filename: str, classes: list[str], title: str, intro: str) -> None:
        ...
        A(intro + "\n")
        A("> **Жирным** выделены значения, отличающиеся от baseline (mode of column). "
          "Так сразу видно, кто из юнитов «особенный» в этой колонке.\n")
        # Collect rows
        rows = []
        for cls in classes:
        ...
        A("| Юнит | Нация | HP | Time | F | G | I | dmg | rng (t) | reload | пика | меч | пуля | картечь | стрела | ядро | uniqueness |")
        A("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
        for r in flat_rows:
    ...
    write_unit_compare("pikemen.md", ["Pikemen 17c"], "Пикинеры 17 век",
                        "Базовая melee пехота с pike-оружием. Эффективны против кавалерии (высокий cannister-protection).")
    write_unit_compare("pikemen18.md", ["Pikemen 18c"], "Пикинеры 18 век",
                        "Поздние пикинеры с улучшенной броней.")
    write_unit_compare("light_infantry.md", ["Light Infantry"], "Light Infantry",
                        "Лёгкая sword/saber пехота. Дешевле пикинеров, слабее против кавалерии.")
    write_unit_compare("musketeers17.md", ["Musketeers 17c"], "Мушкетёры 17 век",
                        "Bullet-стрелки. У каждой нации свой вариант. Strelet (rus), Janissary (tur), Serdiuk (ukr).")
    write_unit_compare("musketeers18.md", ["Musketeers 18c"], "Мушкетёры 18 век",
                        "Поздние мушкетёры — выше damage, лучше броня.")
    ...
    write_unit_compare("archers.md", ["Archers"], "Лучники",
                        "Bow/arrow. Выгоднее против тяжёлой пехоты с низким arrow-protection.")
    write_unit_compare("light_cavalry.md", ["Light Cavalry"], "Лёгкая кавалерия",
                        "Saber/spear лёгкая конница, fasthorse speed=96.")
    write_unit_compare("dragoons.md", ["Dragoons"], "Драгуны",
                        "Mounted shooters. Hit-and-run.")
    ...
    write_unit_compare("peasants.md", ["Peasant"], "Крестьяне",
                        ...
                        "Различия в визуале и стартовых HP.")
    ...
    A("Все уникальные `weaponsid` (типы снарядов и метательного оружия) с их параметрами и "
      ...
      "они задаются в самом weapon-объекте (см. `weapon.script`).\n")
    ...
    A("- **`SHOTMUSKET`** — стандартный мушкетный выстрел. Используется большинством мушкетёров и "
      "драгунов. Стрелец имеет больший урон (9 vs 8) при таком же снаряде.")
    A("- **`STRELA`** / **`OSTRELA`** — обычная стрела и зажигательная. Зажигательная (OSTRELA, "
      "kind=`firearrow`) — второй weapon slot у лучников.")
    A("- **`PPOINTTKOR`** — корабельное ядро (используется фрегатом, ксебеком, баттлшипом, чайкой, "
      "галерой, яхтой).")
    A("- **`PPOINTT`** vs **`PPOINTTFRAME`** — стандартное пушечное ядро vs framegun-ядро.")
    ...
    A("\nИсточник определений: `data/scripts/lib/weapon.script` (функция `_weapon_AddWeapon`). "
      ...
      "не выгружены — см. raw файл при необходимости.")
    ...
    def write_building_compare(filename: str, sid_suffix: str, title: str, intro: str) -> None:
        ...
        A("[← compare/](README.md) · [← Index](../README.md)\n")
        A(intro + "\n")
        A("> **Жирным** — отклонения от baseline (mode of column).\n")
        ...
        baselines = compute_baselines(rows, baseline_cols)
        A("| Здание | Нация | HP | Time (s) | cost% | W | S | G | farm | produces |")
        A("|---|---|---:|---:|---:|---:|---:|---:|---:|---|")
        for b in rows:
            produces = b.get("produces") or []
            ...
            A(f"| {name_cell_short(b)} | {b['nation']} "
              ...
              f"| {prod_str or '—'} |")
        A("")
        A("**Baseline:** " + ", ".join(f"{k}={v}" for k, v in baselines.items()
                                          if v is not None))
        write_md(cmp_dir / filename, out)

    write_building_compare("town_halls.md", "cen", "Town Halls",
                            "Главные здания всех 21 нации. Австрийская строится в 4.69 секунд "
    ...
    A("# Barracks (17c & 18c)\n")
    A("[← compare/](README.md) · [← Index](../README.md)\n")
    A("Бараки тренируют пехоту. У России специфический Strelets Barracks; у Украины — Cossack House.\n")
    A("> **Жирным** — отклонения от baseline.\n")
    for sid_suffix, title, note in [
        ("bar", "Barracks 17c (`<nat>bar`)", ""),
        ("ba2", "Barracks 18c (`<nat>ba2`)", "Не у всех наций (нет у `ukr`/`tur`/`alg`)."),
    ]:
        A(f"## {title}\n")
        if note:
            A(note + "\n")
        ...
        baseline_cols = ["hp", "buildtime_sec", "costpercent", "wood", "stone", "gold", "farm"]
        baselines = compute_baselines(rows, baseline_cols)
        A("| Здание | Нация | HP | Time | cost% | W | S | G | farm | produces |")
        A("|---|---|---:|---:|---:|---:|---:|---:|---:|---|")
        for b in rows:
            produces = b.get("produces") or []
            prod_str = ", ".join(produces[:5]) + (f" (+{len(produces)-5})" if len(produces) > 5 else "")
