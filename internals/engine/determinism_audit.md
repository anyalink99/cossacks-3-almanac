...
> **Важно (RE-валидировано):** оба генератора пишут и читают **одну
> и ту же** глобальную 64-битную seed-ячейку. Это два разных алгоритма
> (32-бит LCG у `random`, 64-бит LCG у `RandomExt`) поверх общего
> состояния. Любой вызов любого из них продвигает seed, который
> увидит следующий. Параллельных «потоков» с раздельным состоянием
> между этими двумя нет. Детали — [`rng_implementation.md` §3, §8](rng_implementation.md);
> разбор по бинарю — в приватном `cossacks-deep/findings/rng_implementation.md`.
