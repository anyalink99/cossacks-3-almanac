...
> **Важно (RE-валидировано):** генераторы используют **разные**
> seed-ячейки. `random` — обёртка над `System._Random`, которая мутирует
> стандартный Delphi `System.RandSeed` (32 бит). `RandomExt` — 64-битный
> LCG над **отдельной** ячейкой, которой управляют `SetRandomKey` и
> `SetRandomExtKey64`. Так что вызов `random` не сдвигает seed
> `RandomExt`, и наоборот. Любопытная тонкость именования: `SetRandomKey`
> на самом деле управляет seed'ом `RandomExt`, не `Random` — детали в
> [`rng_implementation.md` §3](rng_implementation.md). Разбор по бинарю —
> в приватном `cossacks-deep/findings/rng_implementation.md`.
