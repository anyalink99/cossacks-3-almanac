# Реализация RNG в Cossacks 3

Что именно генерирует `random`, `RandomExt` и почему `SetRandomKey`
работает так, как работает.

> Сопровождает [`determinism_audit.md`](determinism_audit.md) (где
> RNG используется в горячем пути добычи и боя) и
> [`native_api.md` §2.4](native_api.md) (полный список
> RNG-потоков движка).

> **Этот документ синхронизирован с RE-проверкой в приватном
> репозитории `cossacks-deep` (`findings/rng_implementation.md`),
> где разобраны конкретные адреса, layout seed-структур и точные
> формулы алгоритмов.** Версия §1–§10 ниже — поведенческая, без
> публикации байтов.

> **История правок.** В одной из ранних итераций (18 мая) этот документ
> ошибочно «поправил» оригинальную модель утверждением, что `Random` и
> `RandomExt` делят общий 64-битный seed. Это было неверно: декомпиляция
> `_System_Random` показала отдельную ячейку `RandSeed` (та, что в Delphi
> System unit), независимую от 64-битного хранилища, которое ставится
> через `SetRandomKey` / `SetRandomExtKey64`. Поэтому оригинальное
> наблюдение (Random и RandomExt — независимые потоки) остаётся в силе.
> Все «Поправлено по RE»-блоки, кажущиеся неуверенными, отражают
> промежуточные итерации; финальная картина — здесь и в `native_api.md` §2.4
> (4 независимых seed-хранилища).

## 1. Общая картина

В скриптах C3 используются (по убыванию частоты):

| Функция | Вызовов в скриптах | Что |
|---|---:|---|
| `random` | 100 | Float ∈ [0, 1), глобальный поток |
| `RandomExt` | 60 | Float ∈ [0, 1), параллельный «расширенный» поток |
| `SetRandomKey(seed: Integer)` | 4 | Перезасев глобального потока |
| `SetRandomExtKey64(k0, k1)` | 0 | Перезасев расширенного 64-bit потока (в exe есть, скрипты не зовут) |

Каждая из них — **обёртка над стандартным Delphi `Random`/`RandSeed`**,
зарегистрированная DWS-движком. Об этом говорит сам факт, что DWS — это
Delphi-библиотека, а не самостоятельная VM, и она наследует RNG
от хоста.

## 2. Алгоритм Delphi `Random`

`System.Random` в Delphi — линейный конгруэнтный генератор (LCG):

```pascal
RandSeed := RandSeed * $08088405 + 1;
// $08088405 = 134775813
// аддент = 1
// модуль = 2^32 (естественное переполнение Cardinal)
```

Преобразование в float:

```pascal
function Random: Double;
begin
    Result := RandSeed / $FFFFFFFF;
end;
```

Свойства:
- **Период:** 2³² ≈ 4.29 × 10⁹ значений.
- **Семя:** 32-bit `Integer` через `SetRandomKey(seed)`.
- **Один глобальный `RandSeed`** на весь процесс (не per-thread).

Это **широко известная** константа Delphi (та же, что в Borland
Pascal с конца 80-х). Ничего эзотерического.

## 3. Реализация `RandomExt`

`RandomExt` — это **64-битный LCG**, не Xorshift. Алгоритм:
`seed64 := seed64 * M + I` со специфическими константами, выполняется
как 64-битная арифметика на 32-битной ISA через RTL-хелперы Delphi
(`__llmul` / `__lladd`).

**Состояние** хранится в отдельной 64-битной ячейке (далее — «расширенный
seed»), которой управляют:

- `SetRandomKey(key: Integer)` — пишет младшую половину = `key`,
  старшую через sign-extend (= 0 для положительных, = −1 = `0xFFFFFFFF`
  для отрицательных).
- `SetRandomExtKey64(const key0, key1: Integer)` — пишет всю 64-битную
  ячейку целиком (низ = `key0`, верх = `key1`).

**`Random` использует другую ячейку** — стандартный 32-битный Delphi
`System.RandSeed`. Это **другая** глобальная переменная. Так что
`Random` и `RandomExt` действительно сидят на независимых seed'ах.

Из этого вытекает важная тонкость именования: имя `SetRandomKey` звучит
так, будто оно ставит seed для `Random()`, но **на самом деле управляет
расширенным seed'ом** (тем же, который читает `RandomExt`). Чтобы засеять
сам `Random`, нужен либо Delphi-овский `Randomize`, либо прямая запись
в `System.RandSeed` — DWS такой возможности не выставляет. То есть
скриптовый паттерн «`SetRandomKey(N); arg.frnd := random;`», встречающийся
в `lib/unit.script`, синхронизирует не `random`, а следующий вызов
`RandomExt` — но автор скрипта мог рассчитывать на другое поведение.

Подробности с адресами и декомпиляцией — в приватном
`cossacks-deep/findings/rng_implementation.md` §§ 1–3.

## 4. Семантика «глобальный RNG»

Из `determinism_audit.md` мы знали, что `random` — глобальный, шарится
между:

- gameplay-логикой (выбор цели, хедшоты, размещение пеньков),
- UI / GUI-эффектами (`gui.script` зовёт `random` для случайного
  материала combobox'а, например),
- ИИ-решениями.

Это означает: **GUI «крадёт» энтропию у gameplay**, и наоборот. Если
два клиента в сетевой игре проигрывают по-разному воспроизводимый
GUI, их глобальные `RandSeed` расходятся, и gameplay тоже разойдётся.

Защита идёт через **локальный пересев** перед gameplay-критичной
операцией (см. §5) — `SetRandomKey` ставит расширенный seed, и
последующая серия `RandomExt`-вызовов становится воспроизводимой
независимо от того, что делает GUI с `random`. Visual/UI-код, который
использует только `random` (не `RandomExt`), мутирует только
`System.RandSeed`, не задевая расширенный seed — это и есть фактическое
разделение, благодаря которому gameplay сохраняет детерминизм.

Отдельные именованные потоки с собственным state'ом — `AirWeatherRandom`,
`MakeRandomClouds`-производные, map-генератор'ы — живут полностью вне
обеих общих seed-ячеек.

## 5. Главный паттерн детерминизма: пересев перед операцией

В скриптах `lib/unit.script` 4 вызова `SetRandomKey` — все
сделаны **специально для синхронизации**. Контекст из кода:

```pascal
// unit.script — перед формированием отряда
SetRandomKey(floor(random * gc_MaxInt));
// needed to sync multiplayer arg.frnd
arg.frnd := random;
```

```pascal
// unit.script — перед расчётом, зависящим от персонального юнита
SetRandomKey(floor(TObj(pobj).uniqrnd * gc_MaxInt));
// sync multiplayer
```

**Архитектурный паттерн:** перед операцией, требующей одинакового
результата на всех клиентах, скрипт **пересеивает глобальный RNG**
из источника, который **гарантированно одинаков на всех клиентах**:

| Источник пересева | Откуда детерминирован |
|---|---|
| `random` (предыдущее значение) | Если до этого синхронизация уже была — то одинакова. |
| `TObj(pobj).uniqrnd * gc_MaxInt` | `uniqrnd` юнита — фиксирован при спавне и **синхронно сохраняется** (см. поле `uniqrnd` в [`server_sync_packet_format.md` §3.1](server_sync_packet_format.md)). |

Это **не lockstep**, а **per-decision deterministic seed**. То же
самое решение, принятое на сервере и на клиенте (например, «какая
формация юнитов в отряде»), даст бит-в-бит одинаковый результат, не
требуя синхронизированного RNG-состояния.

## 6. Воспроизводимость значений

С учётом §2 (LCG) — задав `SetRandomKey(seed)`, можно полностью
воспроизвести следующие N значений `random`. Простая Python-модель:

```python
def delphi_random_stream(seed):
    s = seed & 0xFFFFFFFF
    while True:
        s = (s * 134775813 + 1) & 0xFFFFFFFF
        yield s / 0xFFFFFFFF

# Например, для seed=42:
g = delphi_random_stream(42)
print([next(g) for _ in range(5)])
# [0.5263867462985266, 0.4017018212098335, 0.7770079020410776, ...]
```

Это значит — **для каждого `uniqrnd` юнита можно заранее предсказать
любую RNG-зависимую величину** (хедшот, разлёт снаряда, выбор цели
из равных кандидатов). Симулятор может это использовать для
повторения боёв из save'ов.

## 7. Что насчёт MT-19937

Стандартный DWS (на GitHub) предоставляет **и LCG (`Random`), и
Mersenne Twister** (через `dwsRandomFunctions.pas`). Но в C3 имя
`Random` в exe регистрируется как **обычный 32-bit LCG**, что
подтверждается декомпиляцией: `Random()` оборачивает `System._Random`
с классической Delphi-формулой `seed := seed * 0x8088405 + 1`.

Поэтому, несмотря на то что MT-19937 в DWS доступен, в C3 он **не
используется**. `RandomExt` — тоже не MT, а 64-битный LCG над **своим
отдельным** расширенным seed'ом (см. §3).

## 8. Связь с другими RNG-потоками

В движке **четыре независимых seed-хранилища**, каждое со своим
алгоритмом сверху:

```
Стандартный Delphi RandSeed (32-бит)
                        ↑
                  Random  (Delphi 32-бит LCG: seed = seed * 0x8088405 + 1)
                        (Randomize / прямая запись System.RandSeed — DWS не выставляет)

Расширенный 64-бит seed
                        ↑
                  RandomExt  (64-бит LCG со своей парой констант)
                        ↑
                  SetRandomKey (sign-extend 32→64), SetRandomExtKey64 (полный 64)

MapGenerator seed       (64-бит, отдельное хранилище в state карты)
                        ↑
                  SetMapGeneratorRandomKey

GlobalMapGenerator seed (64-бит, отдельное хранилище)
                        ↑
                  SetGlobalMapGeneratorRandomKey

(Отдельные каналы для погоды и облаков — AirWeatherRandom-семейство, не показано)
```

Все четыре хранилища **физически разные**: вызов любого алгоритма
влияет только на свой seed. Это объясняет, почему пара `(randkey0,
randkey1)` для воспроизведения карты — стандартное представление в
save и lobby (см.
[`map_generation_pipeline.md`](../../docs/recon/world/map/map_generation_pipeline.md) §12)
и почему gameplay-критичные пути в скриптах оборачиваются в
`SetRandomKey + RandomExt` — это гарантирует детерминизм
расширенного потока независимо от того, что делает UI с обычным
`random`.

## 9. Что это значит для симулятора

Для модели extraction (см.
[`docs/recon/world/peasant_extraction.md`](../../docs/recon/world/economy/peasant_extraction.md)),
если мы хотим **бит-точно** воспроизвести добычу с заданного save'а:

1. Считать `RandSeed` нельзя — он не сохраняется как отдельное поле.
2. Вместо этого считаем `uniqrnd` каждого крестьянина (есть в
   sync-снимке).
3. Для каждой RNG-зависимой операции в hot-loop'е (например, выбор
   точки на дереве) симулятор должен **знать пересевы** в скрипте и
   повторить их.
4. Реальная реализация — пройти `lib/unit.script` `_unit_DoExtract`
   и для каждого `random`-вызова определить, был ли там
   `SetRandomKey` непосредственно перед.

Это и есть план для симулятора Level C
[`project_level_c_simulator_plan.md`](../project/architecture.md)
(если симулятор когда-нибудь будет нацелен на bit-perfect
reproducibility, а не на статистическую точность).

## 10. Открытое и закрытое

| Пункт | Статус |
|---|---|
| **Точная реализация `RandomExt`** | **Закрыто.** 64-битный LCG на отдельной 64-битной seed-ячейке (не на той же, что у `Random`). См. §3, §8 и приватный `cossacks-deep/findings/rng_implementation.md`. |
| **Связь `SetRandomKey` и `Random`** | **Закрыто.** `SetRandomKey` управляет расширенным seed'ом (= seed `RandomExt`), не стандартным `RandSeed`. Несмотря на имя, `Random()` не зависит от `SetRandomKey` — см. §3. |
| **Алгоритмы `MapGenerator` / `GlobalMapGenerator`** | Частично. Seed-хранилища подтверждены отдельные (см. §8). Сами алгоритмы для генерации значений не разобраны; будут пройдены, когда возьмёмся за `_DoGenerate`. |
| **`PlayerCubeRandomValue`** | Частично. Не мутирует никакой RNG-seed — это **пре-вычисленный per-player nonce**, заранее сгенерированный (вероятно при коннекте игрока) и стабильный весь матч. Точный момент инициализации не подтверждён. Подробнее: приватный `findings/rng_implementation.md`. |
| **Использует ли `_DoGenerate` обычный `Random()`** (что молча мутировало бы `System.RandSeed` во время генерации карты) | Открыто. Следующий шаг по RNG-теме. |
| **Синхронизируется ли `System.RandSeed` по сети** на старте матча | Открыто. Кандидаты — handler'ы пакетов `EconomyPackage`/sync. |
| **Race condition между `PathDataThread*` и основным `bProcess` на RNG-вызовах** | Открыто. Главный подозреваемый по необъяснённым рассинхронам. |
